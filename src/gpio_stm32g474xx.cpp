...
void GPIO::pin_config_impl(const GPIO_Pin &pin_num, const GPIO_Mode &mode) {
  ...
  gpio->MODER = gpio->MODER & ~(0b11 << offset); // reset to 0
  gpio->MODER = gpio->MODER | (mode << offset);  // set to desired mode
}

void GPIO::pin_config_impl(const GPIO_Pin &pin_num, const GPIO_OutputType &otype) {
  const int offset = pin_num * 1;
  gpio->OTYPER = gpio->OTYPER & ~(0b1 << offset); // reset to 0
  gpio->OTYPER = gpio->OTYPER | (otype << offset);
}

void GPIO::pin_config_impl(const GPIO_Pin &pin_num, const GPIO_Speed &ospeed) {
  const int offset = pin_num * 2;
  gpio->OSPEEDR = gpio->OSPEEDR & ~(0b11 << offset); // reset to 0
  gpio->OSPEEDR = gpio->OSPEEDR | (ospeed << offset);
}

void GPIO::pin_config_impl(const GPIO_Pin &pin_num, const GPIO_PullUpDown &pupd) {
  const int offset = pin_num * 2;
  gpio->PUPDR = gpio->PUPDR & ~(0b11 << offset); // reset to 0
  gpio->PUPDR = gpio->PUPDR | (pupd << offset);
}
...
void GPIO::pin_config_impl(const GPIO_Pin &pin_num, const GPIO_AlternateFunction &af) {
  ...
  if (pin_num < 8) {
    gpio->AFR[0] = gpio->AFR[0] & ~(0b1111 << offset1); // reset to 0
    gpio->AFR[0] = gpio->AFR[0] | (af << offset1);
  } else {
    gpio->AFR[1] = gpio->AFR[1] & ~(0b1111 << offset2); // reset to 0
    gpio->AFR[1] = gpio->AFR[1] | (af << offset2);
  }
}
} // namespace stmlibxx
